...
export const getUserCartOrder = async (req, res) => {
    ...
    try {
        ...
        const cartOrder = await OrderModel.find({ userId: params.userId, status: { $in: ["PENDING"] }, isDeleted: false }).populate("productId", "name _id productPageImageUrl").sort({ $natural: -1 }).skip(skips).limit(limit)
